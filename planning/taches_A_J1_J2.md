...
- `worldState.Players.Remove(client.Id)` ajouté dans le `lock(stateLock)` au disconnect

### ✅ Fix : thread safety HandleJoin
- `ContainsKey` et limite 4 joueurs déplacés à l'intérieur du `lock(stateLock)`
...
- Partagé — D a pu construire sa table de routage (`planning/routing_table_D.md`)
